...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let processedMessageIds = new Set(); // Set to track processed message IDs to avoid duplicates
  let reportedMessages = new Set(); // Track reported messages
  let bannedUsers = new Set(); // Track banned users
  let userReportCounts = new Map(); // Track number of reports per user
  ...
  completedTutorial: false // Add new setting to track tutorial completion
  };
  ...
  // Add report confirmation to the message
  ...
  if (userReportCounts.get(reportedUser) >= 5 && !bannedUsers.has(reportedUser)) {
      ...
      // Update UI to show user as banned
  ...
  // Add this code to the sendMessage() function right after the socket.emit calls
// This goes inside the sendMessage() function, before the messageInput.value = ''; line

  if (chatMode === 'server' && activeServer && message) {
    // Create timestamp for immediate local rendering
    const timestamp = Date.now();
    const messageId = generateMessageId(username, timestamp, message);
    
    // Skip if we've already processed this message
    if (!processedMessageIds.has(messageId)) {
      processedMessageIds.add(messageId);
      
      // Create and add message to UI immediately
  ...
  } else if (chatMode === 'dm' && activeDm && message) {
    // Create timestamp for immediate local rendering
    const timestamp = Date.now();
    const messageId = generateMessageId(username, timestamp, message);
  ...
  // Add reported class if message is reported
  ...
  if (isFirstTimeUser) {
      // Allow the app UI to load first
      setTimeout(() => {
          showTutorial();
      }, 1000);
  }
  ...
  const messageId = generateMessageId(msgUsername, timestamp, message);
  ...
  // Create unique message ID
  const messageId = generateMessageId(from, timestamp, message);
